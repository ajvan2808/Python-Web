...
def tra_cuu_san_pham(chuoi_tu_khoa, danh_sach_san_pham):
    danh_sach_ket_qua = list(filter(lambda san_pham: chuoi_tu_khoa.lower() in san_pham['Ten'].lower(), danh_sach_san_pham))
    return danh_sach_ket_qua

def lay_thong_tin_danh_muc(danh_sach_san_pham):     # lấy các thông tin trong list Danh_muc từ list san_pham.json
    ds_danh_muc = {}
...
def tim_ma_san_pham_moi_nhat(danh_sach_san_pham):       
    ds_ma_so = []
    for san_pham in danh_sach_san_pham:
        ds_ma_so.append(san_pham['Ma_so'])     # mã số sản phẩm lớn nhất là sớm phẩm mới nhất được cập nhật
    return max(ds_ma_so)
...
def tao_chuoi_html_danh_sach_sp(danh_sach_san_pham):
    ...
    <div class="col">
		<div class="row-4" style="margin-bottom: 10px;">
			<a class="btn btn-outline-primary" href="/dang-ky-thanh-vien" role="button">
            Đăng ký thành viên 
            </a>
		</div>

		<div class="row-4" style="margin-bottom: 5px;">
			<a class="btn btn-outline-primary" href="/gui-y-kien" role="button">
            Gửi ý kiến
            </a>
		</div>
	</div>
...
def tao_chuoi_html_danh_sach_sp_admin(danh_sach_san_pham):
	...
	<a class="btn btn-primary" href="/admin/them-san-pham" role="button"> Thêm sản phẩm </a>
	</div>
    '''
    ...
    for san_pham in danh_sach_san_pham:
        chuoi_html += '''
			<tr>
				...
				<td>
                    <img src="/static/san-pham/'''+ san_pham['Hinh_anh'] +''' " height="80" alt=" '''+ san_pham['Hinh_anh'] +''' ">
		            ...
		            <div class="modal-content">
                        ...
                        <!-- cần nhớ bỏ trong form POST và type là submit dành cho button Đồng ý -->
                            ...
                            <form method="POST">
                                ...
                                <input hidden type="text" name="TenHinh_Xoa" value="'''+ san_pham['Hinh_anh'] +'''" />
                            </form>
                        </div>
